  ...
  constructor($http, $scope, $stateParams, appConfig, cfpLoadingBar, $location, localStorageService, $state) {
    ...
    $scope.submit = function(){
      ...
      $http(req).then(response => {
        ...
        var data = {
          "password": "thepass",
          "method": "setUpContractDetails",
          "args": {
            "contractName": $scope.contractContract.contractName,
			"contractPrice": $scope.contractContract.contractPrice,
			"products": $scope.contractContract.products
			
			
            //,
            //"oracleAddress": $scope.contractContract.oracleAddress
          },
          "contract": "Contract"
        };
        ...
        var req = {
         method: 'POST',
         url: appConfig.keyserver + 'users/' + localStorageService.get('user')+ '/'+ localStorageService.get('address') + '/contract/Contract/' + $scope.newContract + '/call',
         headers: {
           'Content-Type': 'application/json'
         },
...
angular.module('microsaas')
  .controller('Swap6Controller', Swap6Controller);
